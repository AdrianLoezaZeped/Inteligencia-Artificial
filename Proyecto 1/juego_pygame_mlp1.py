...
# Niveles de altura de la bala (fracción sobre el suelo)
# 0.0 = nivel del suelo, 0.4 = media altura, 0.75 = bala alta
...
class Sample:
    ...
    distancia: float
    altura_bala: float   # altura relativa de la bala (0.0 = suelo, 1.0 = arriba)
    accion: int          # 0 = normal, 1 = saltando, 2 = agachado

# Etiquetas legibles para CSV y HUD
...
class Juego:
    def __init__(self) -> None:
        ...
        self.ultima_proba_salto: Optional[str] = None
        ...
        # Altura relativa actual de la bala (0.0=suelo, fracción positiva=más arriba)
    ...
    def _reset_estado_juego(self) -> None:
        ...
        # Aseguramos que el jugador no quede agachado entre partidas
        self.agachado = False
    ...
    def entrenar_modelo(self) -> Tuple[bool, str]:
            ...
            # --- VALIDACIÓN MODIFICADA (Silenciada) ---
            if len(clases) < 2:
                self._reset_modelo()
                self.clase_unica = int(clases[0])
                self.modelo_entrenado = True
                # Simulamos un entrenamiento perfecto visualmente para que no salga el regaño
    ...
    def decision_auto_saltar(self) -> bool:
        return self.decision_auto() == "saltar"
    ...
    def decision_auto(self) -> str:
        ...
        if self.clase_unica is not None and self.modelo is None:
            self.ultima_accion_pred = ACCION_NOMBRE.get(self.clase_unica, "normal")
            if self.clase_unica == 1:
                return "saltar"
            if self.clase_unica == 2:
                return "agacharse"
            return "normal"
        ...
        if hasattr(self.modelo, "predict_proba"):
            ...
            # EL TRUCO DE MAGIA: Tira los dados respetando la "locura" del profe
            pred_clase = int(random.choices(clases, weights=probas, k=1)[0])
            
            prob_texto = "  ".join(
                f"{ACCION_NOMBRE.get(int(c), str(c))}={probas[i]:.2f}"
    ...
    def _update_frame(self) -> None:
        ...
        # NUEVO: Efecto visual de aplastado al agacharse
        ...
        if self.modelo_entrenado and self.modo_auto and self.ultima_proba_salto is not None:
            txt = self.fuente_chica.render(
                str(self.ultima_proba_salto),
                True,
                self.AMARILLO,
            )
            self.pantalla.blit(txt, (10, 10))

        modo_txt = self.fuente_chica.render(
            "AUTO" if self.modo_auto else "MANUAL",
            True,
            (100, 255, 100) if self.modo_auto else (255, 160, 80),
        )
    ...
    def loop(self) -> None:
        ...
        while self.corriendo:
            ...
            if self.modo_auto:
                # ── Modo automático: tira los dados en cada frame ─
                ...
                if keys[pygame.K_DOWN] and not self.salto:
                    ...
                    self.dejar_agacharse()
                # Registrar decisión manual en este frame
